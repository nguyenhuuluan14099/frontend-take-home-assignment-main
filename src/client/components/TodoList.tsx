...
export const TodoList = ({ tab }: { tab: string }) => {
  ...
  const getFilteredTasks = () => {
    ...
    return todos.filter((todo: Todo) => todo.status === tab)
  }

  return (
    <ul ref={parent} className="grid grid-cols-1 gap-y-3">
      {getFilteredTasks().map((todo: Todo) => (
        <li key={todo.id}>
          <div className="flex items-center justify-between rounded-12 border border-gray-200 px-4 py-3 shadow-sm">
            <div className="flex items-center">
              <Checkbox.Root
                ...
                id={String(todo.id)}
                className={`flex h-6 w-6 items-center justify-center rounded-6 border border-gray-200   focus:border-gray-700 focus:outline-none data-[state=checked]:border-gray-700 data-[state=checked]:bg-gray-700  `}
              >
                <Checkbox.Indicator>
            ...
            <div onClick={() => handleDeleteTodo(todo.id)}>
              <XMarkIcon className=" h-8 w-8 cursor-pointer rounded-12 text-gray-700"></XMarkIcon>
            </div>
          </div>
        </li>
      ))}
